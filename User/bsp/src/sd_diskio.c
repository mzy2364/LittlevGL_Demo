...
DRESULT SD_read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
	...
	res = BSP_SD_ReadDisk_DMA(buff,sector,count);
	//res = BSP_SD_ReadDisk(buff,sector,count);
...
{
	DRESULT res = RES_ERROR;
	res = BSP_SD_WriteDisk_DMA(buff,sector,count);
	//res = BSP_SD_WriteDisk((uint8_t *)buff,sector,count);
	return res;
}
#endif /* _USE_WRITE == 1 */  
