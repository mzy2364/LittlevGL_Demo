...
//static uint8_t __g_font_buf[352];//如bin文件存在SPI FLASH可使用此buff


static uint8_t *__user_font_getdata(int offset, int size){
    //如字模保存在SPI FLASH, SPIFLASH_Read(__g_font_buf,offset,size);
    //如字模已加载到SDRAM,直接返回偏移地址即可如:return (uint8_t*)(sdram_fontddr+offset);
    return (uint8_t*)(LV_USER_FONT_ADDR + LV_USER_FONT_OFFSET_24 + offset);
}
...
//阿里巴巴普惠体 R,Regular,24
//字模高度：44
//XBF字体,外部bin文件
lv_font_t gb2312_puhui24 = {
